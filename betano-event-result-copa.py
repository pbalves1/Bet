...
#
# Modelo de resposta do html do site https://vsa2.gambling-malta.com/vr-media-schedule/MediaViewer?sportType=FOOTBALL_MATCH&operator=betanoWorldCup
#
...
def getVideo(contentJson, idVideo):
    ...
    conn.request("GET", "/media/viewer/vanilla/vanilla-viewer.php?userId=1&operator=betanoWorldCup&eventId="+str(idVideo), payload, headers)
    ...
    if(parser.homeTeam == contentJson['data']['currentEvents'][0]['participants'][0]['name'] or parser.awayTeam == contentJson['data']['currentEvents'][0]['participants'][1]['name']):
        parser.correctTeam = True

    return parser

def callBetanoApi(id):
    contents = urllib.request.urlopen("https://br.betano.com/api/virtuals/event/"+id+"?sportId=FOOT&regionId=189800&req=la,stnf,c").read()
    ...
    if 'id' in itemDynamo:
        ...
        while apiCallTry:
            try:
                ...
                time.sleep(4)
        if(contentJson['data']['currentEvents'][0]['participants'][0]['name'][0:3] == 'Col'):
            contentJson['data']['currentEvents'][0]['participants'][0]['name'] = 'Colombia'
        if(contentJson['data']['currentEvents'][0]['participants'][1]['name'][0:3] == 'Col'):
            contentJson['data']['currentEvents'][0]['participants'][1]['name'] = 'Colombia'

        itemDynamo['homeTeam'] = {'S': contentJson['data']['currentEvents'][0]['participants'][0]['name']}
        itemDynamo['awayTeam'] = {'S': contentJson['data']['currentEvents'][0]['participants'][1]['name']}
        ...
        if contentJson['data']['currentEvents'][0]['liveNow']:
            ...
            conn.request("GET", "/vr-media-schedule/MediaViewer?sportType=FOOTBALL_MATCH&operator=betanoWorldCup", payload, headers)
            res = conn.getresponse()
            data = res.read()
